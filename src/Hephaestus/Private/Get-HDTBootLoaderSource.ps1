function Get-HDTBootLoaderSource {
    <#
        ...
        .DESCRIPTION
            ...
            (SPIKES S20.2 and S20.4). Six Generation 2 Hyper-V runs across two
            builds, and no build has ever produced swapped media that starts:
            ...
            10.0.28000.342. 0xc0430001 is STATUS_SECUREBOOT_ROLLBACK_DETECTED,
            and it arrives on Windows Boot Manager's recovery screen rather than
            a firmware Security Violation. It is NOT gated on Secure Boot: the
            2026-09-10 media refused with the firmware's Secure Boot off too.
            winload.efi carries no BOOTMGRSECURITYVERSIONNUMBER resource at all,
            so nothing about it can be corrected by copying files.

            AND COPYING THE OS LOADER IN WAS TRIED, AND IT IS WORSE. 0.24.0
            replaced winload.efi inside the mount from the same patched Windows
            the boot managers came from. The replacement was verified
            byte-identical inside the shipped WIM and the media went from
            booting in one configuration to booting in none - the Secure-Boot-OFF
            run that PASSED in S20.2 FAILED in S20.4. It is removed. A loader
            and ntoskrnl.exe are a matched pair; only servicing moves both.

            Assert-HDTBootLoaderServicingLevel is the guard that refuses an
            unserviced image. It is a guard and not a cure.
            ...
            than overwriting files in the media tree:
              ...
              1. Apply the current Windows cumulative update (.msu) to the
                 ...
                 same mount step 7 already opens. That is what moves winload.efi
                 and ntoskrnl.exe together, which a file copy cannot do. It will
                 NOT produce a 28000-series loader and is not meant to: 28000 is
                 the boot manager's own servicing track, not the OS's.
              ...
              4. Prove it the only way it can be proved: a Generation 2 VM with
                 ...
                 and in S20.4 every pre-boot check on the copy-the-loader build
                 passed too, hashes included, and the machine still refused it.
