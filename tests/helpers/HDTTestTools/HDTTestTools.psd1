@{
    RootModule           = 'HDTTestTools.psm1'
    ModuleVersion        = '0.1.0'
    GUID                 = 'b5d9ae60-03f1-4c7a-9ee1-c2dc7be42dc4'
    Author               = 'Itamartz'
    CompanyName          = 'Hephaestus Deployment Toolkit'
    Copyright            = '(c) 2026 Itamartz. All rights reserved.'
    Description          = 'Shared build and test helpers for the Hephaestus Deployment Toolkit.'

    PowerShellVersion    = '5.1'
    CompatiblePSEditions = @('Desktop', 'Core')

    FunctionsToExport    = @(
        'Assert-HDTLabMemoryBudget',
        'Assert-HDTLabScratchDisk',
        'Assert-HDTLabVmName',
        'Assert-HDTLabVmPath',
        'ConvertFrom-HDTThumbnailImage',
        'ConvertTo-HDTCanonicalJson',
        'Get-HDTAutoLogonArtifact',
        'Get-HDTBadgeColor',
        'Get-HDTFunctionNameViolation',
        'Get-HDTLabMemoryBudget',
        'Get-HDTLabMemoryUse',
        'Get-HDTLabOfflineComputerName',
        'Get-HDTLabProtectedVm',
        'Get-HDTLabVmStamp',
        'Get-HDTJsonSchemaInstanceType',
        'Get-HDTJsonSchemaKeyword',
        'Get-HDTJsonSchemaNodeViolation',
        'Get-HDTJsonSchemaViolation',
        'Get-HDTJsonSchemaVocabulary',
        'Get-HDTLogRecord',
        'Get-HDTMdtDependency',
        'Get-HDTRealMachineState',
        'Get-HDTSlowSuiteSkipViolation',
        'Get-HDTSourceFile',
        'Get-HDTScriptCompatibilityViolation',
        'Get-HDTSourceFunction',
        'Get-HDTScratchLeakViolation',
        'Get-HDTScratchRootReference',
        'Get-HDTShardFailureReason',
        'Get-HDTWorkerCount',
        'Merge-HDTPesterSummary',
        'New-HDTBadgeFile',
        'New-HDTLabContentDisk',
        'New-HDTLabScratchDisk',
        'New-HDTLabVirtualMachine',
        'New-HDTPesterConfiguration',
        'New-HDTSequenceTestHarness',
        'Remove-HDTLabScratchDisk',
        'Remove-HDTLabScratchTree',
        'Remove-HDTLabVirtualMachine',
        'Resolve-HDTJsonSchemaReference',
        'Save-HDTLabVmScreen',
        'Split-HDTTestBucket',
        'Send-HDTLabVmText',
        'Test-HDTFunctionName',
        'Test-HDTJsonSchema',
        'Test-HDTLabVmStamped',
        'Test-HDTModuleAvailable',
        'Test-HDTScriptCompatibility',
        'Wait-HDTLabVmState'
    )
    CmdletsToExport      = @()
    VariablesToExport    = @()
    AliasesToExport      = @()

    PrivateData          = @{
        PSData = @{
            Tags = @('HDT', 'Testing', 'Build')
        }
    }
}
