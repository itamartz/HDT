...
"@, $utf8NoBom)
    ...
    if ($script:canRun) {
        ...
        #
        # This suite runs its two VMs one at a time, so one 4 GB reservation is
        # the whole of its claim - which is what the block this replaced did.
        Assert-HDTLabMemoryBudget -MemoryByte 4294967296 -Name $script:buildVmName
