...
| `DiskGuard.E2E.Tests.ps1` | ~10 min | **DESIGN 9.1 rule 5 on hardware — the guard that decides which disk gets wiped.** A GPT disk of three partitions, **not one of them lettered** (an MSR, an ESP, and an NTFS volume carrying `NoDefaultDriveLetter` and a real file), is **refused** by a `DiskPartition` step that did not declare `wipe: true` — the defect commit `d807c60` fixed, proven until now only against fakes. Then the same disk, in the same boot, **is accepted** when the sequence does declare it. The refusal must name *"partitions HDT cannot read … no drive letter"*: that arm exists only after the fix, and a refusal through the other arm means WinPE lettered the fixture |
