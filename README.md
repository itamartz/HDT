...
![Building the reference image](docs/images/hdt-01-reference-build.gif)

<sub>Full resolution: [hdt-01-reference-build.mp4](https://github.com/itamartz/HDT/releases/download/v0.12.0/hdt-01-reference-build.mp4)</sub>
...
![Deploying the captured image](docs/images/hdt-02-deploy-capture.gif)

<sub>Full resolution: [hdt-02-deploy-capture.mp4](https://github.com/itamartz/HDT/releases/download/v0.12.0/hdt-02-deploy-capture.mp4)</sub>
...
| **M7** — capture and standalone media | **both exits met** — capture 2026-08-31, media 2026-09-03. It was scheduled out to v2 and then built; this row said "deferred" for longer than it was true |
| **M8** — console (WPF) | met |
| **M9** — refresh | **met on hardware, 2026-09-11** (0.24.1). A BitLocker-protected machine was refreshed with nothing inserted: the shipped `Scripts\Start-HDTRefresh` launcher, `SuspendBitLocker` on a `FullyEncrypted` volume, an armed one-shot boot entry that reached **WinPE rather than a recovery prompt**, `CleanVolume`, the apply, and a third leg inside the installation it had just written. The partition table came back identical — same disk GUID, same offsets, same identifiers — and every leg recorded a derived `REFRESH`. It took **22 fixes** to get there, seventeen of them in the product: the launcher could not load an assembly over UNC, `Enable-BitLocker` was called with no protector switch and could never have bound, `EnableBitLocker` targeted the WinPE drive letter, `CleanVolume` followed junctions and refused on ReadOnly folders, and `ConfigureBoot` assumed an ESP letter only the partition step creates. |
